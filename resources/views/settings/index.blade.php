...
<div class="sed-settings-stack" style="display:flex;flex-direction:column;gap:1.25rem;max-width:min(56rem,100%);">
    ...
    <div class="sed-card">
        @php($hasSignaturePin = auth()->user()->hasSignaturePin())
        <h2 style="margin-top:0;font-size:1.1rem;">Электронная подпись</h2>
        <form method="post" action="{{ route('settings.signature') }}">
            @csrf
            @if ($hasSignaturePin)
                <div class="sed-field">
                    <label for="sed-settings-current-pin">Текущая подпись <span class="sed-req" aria-hidden="true">*</span></label>
                    <input class="sed-input" type="password" inputmode="numeric" pattern="\d{4,6}" maxlength="6" autocomplete="current-password" id="sed-settings-current-pin" name="current_signature_pin" required placeholder="Текущий код">
                    @error('current_signature_pin')
                        <p class="sed-field-error">{{ $message }}</p>
                    @enderror
                </div>
            @endif
            <div class="sed-field">
                <label for="sed-settings-pin">{{ $hasSignaturePin ? 'Новая подпись' : 'Подпись' }} <span class="sed-req" aria-hidden="true">*</span></label>
            ...
            <div class="sed-field" style="margin-bottom:0;">
                <label for="sed-settings-pin2">Повтор {{ $hasSignaturePin ? 'новой подписи' : 'подписи' }} <span class="sed-req" aria-hidden="true">*</span></label>
            ...
            <div style="margin-top:1rem;">
                <button type="submit" class="sed-btn sed-btn--primary sed-btn--sm">{{ $hasSignaturePin ? 'Сменить подпись' : 'Сохранить подпись' }}</button>
            </div>
        </form>
    </div>
        ...
        <div class="sed-table-wrap">
            <table class="sed-table">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="2">Записей пока нет.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
